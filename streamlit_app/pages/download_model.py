        ...
        try:
            for res in ollama.pull(newmodel_name, stream=True):
                ...
                if "total" in res and "completed" in res:
                    ...
                    my_bar.progress(
                        percent,
                        text=f"Downloading ({completed_in_mib:.1f} MiB / {total_in_mib:.1f} MiB)",
                    )
                else:
            ...
            st.error(
                f'It looks like "**`{newmodel_name}`**" is not the right name.',
                icon="🚨",
            )
...
st.info(
    "Check the model name on [Ollama Library](https://ollama.com/library) page.",
    icon=":material/info:",
)
